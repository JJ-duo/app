...
#         lines = file_lines_to_list(txt_file)  # 读取检测结果文件的每一行

#         # 解析每一行，提取类别名称、置信度和边界框坐标
#         for line in lines:
#             try:
...
"""
# 初始化变量 sum_AP，ap_dictionary 和 lamr_dictionary，用于存储所有类别的AP总和和每个类别的AP和LAMR
sum_AP = 0.0
ap_dictionary = {}
lamr_dictionary = {}
# open file to store the output
# 打开一个文件用于存储输出结果，文件名为 output.txt，位于 output_files_path 目录下
with open(output_files_path + "/output.txt", 'w') as output_file:
    output_file.write("# AP and precision/recall per class\n")
    # 初始化一个字典 count_true_positives 来记录每个类别的真实正例（true positives）的数量
    count_true_positives = {}
    # 遍历所有类别的名称（gt_classes）
    for class_index, class_name in enumerate(gt_classes):
        count_true_positives[class_name] = 0
        """
         Load detection-results of that class
        """
        # 为每个类别加载对应的检测结果文件（dr_file），文件格式为JSON
        ...
        """
        #  计算每个类别的检测结果数量（nd），并初始化两个数组 tp 和 fp 来记录每个检测是否为真正例（true positive）或假正例（false positive）
        nd = len(dr_data)
        ...
        for idx, detection in enumerate(dr_data):
            ...
            if show_animation:
                # find ground truth image 找到对应的真实标注图像
            ...
            # 加载检测结果对应的真实标注文件（gt_file）
            gt_file = TEMP_FILES_PATH + "/" + file_id + "_ground_truth.json"
            ground_truth_data = json.load(open(gt_file))
            # 初始化变量 ovmax 和 gt_match 来记录最大重叠度（IoU）和匹配的真实标注对象
            ovmax = -1
            gt_match = -1
            # load detected object bounding-box
            bb = [ float(x) for x in detection["bbox"].split() ]
            # 遍历真实标注数据，寻找与当前检测结果匹配的标注框
            ...
            # set minimum overlap
            # min_overlap 是用于判断检测框和真实标注框是否匹配的IoU阈值。如果通过命令行参数 --set-class-iou 设置了特定类别的IoU阈值，则使用该值
            ...
            # 如果最大IoU大于或等于最小IoU阈值，则进一步检查：
            # 如果真实标注对象的 "difficult" 键不存在（即该对象不是难以检测的对象），并且没有被标记为已使用，则将其标记为真正例，并更新真实标注对象的 "used" 标记。
            # 如果真实标注对象已经被使用（即之前已经被其他检测框匹配过），则将其标记为假正例
            ...
            #     if class_name not in ground_truth_img:
            #         color = purple
            ...
            #         cv2.putText(img_cumulative, class_name, (bbgt[0],bbgt[1] - 5), font, 0.6, light_blue, 1, cv2.LINE_AA)
            #     bb = [int(i) for i in bb]
            #     #if class_name not in valid_class_names:  # Check against the list of valid class names
            #     cv2.rectangle(img,(bb[0],bb[1]),(bb[2],bb[3]),color,2)
            #     cv2.rectangle(img_cumulative,(bb[0],bb[1]),(bb[2],bb[3]),color,2)
            #     cv2.putText(img_cumulative, class_name, (bb[0],bb[1] - 5), font, 0.6, color, 1, cv2.LINE_AA)
            #     # # show image
